...
class Ball(pygame.sprite.Sprite):
    def __init__(self, screen_width, screen_height, other_sprites, radius=10, color=(255, 255, 255), speed=5):
        ...
        self.image.fill((0, 0, 0, 0))  # Transparent bakgrund
        ...
        for sprite in collisions:
            ...
            if sprite != self:
                ...
                if self.rect.centerx < sprite.rect.centerx:
                    self.direction[0] = random.uniform(-1, -0.5)  # Slumpmässig förändring till vänster
                else:
                    self.direction[0] = random.uniform(0.5, 1)  # Slumpmässig förändring till höger
                if self.rect.centery < sprite.rect.centery:
                    self.direction[1] = random.uniform(-1, -0.5)  # Slumpmässig förändring uppåt
                else:
                    self.direction[1] = random.uniform(0.5, 1)  # Slumpmässig förändring nedåt
